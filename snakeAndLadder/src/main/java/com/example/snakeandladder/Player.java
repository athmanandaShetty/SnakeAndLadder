...
public class Player {
   ...
   private static Board gameBoard = new Board();
      ...
      public void  movePlayer(int diceValue){ //moving player
          if((currentPosition + diceValue) <= 100){
              currentPosition += diceValue;

              TranslateTransition secondMove = null, firstMove = transalteAnimation(diceValue);

              int newPosition =  gameBoard.getNewPosition(currentPosition);
              if(newPosition != currentPosition && newPosition != -1){
                  currentPosition = newPosition;
                 secondMove = transalteAnimation(6);
...
//          coin.setTranslateY(y);
      ...
      private TranslateTransition transalteAnimation(int diceValue){ //moving coin slowly
